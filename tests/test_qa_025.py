...
def mixed_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vhost_helper.scaffolding.run_elevated_command", mock_run_elevated)
    monkeypatch.setattr("vhost_helper.permissions.run_elevated_command", mock_run_elevated)
    monkeypatch.setattr("vhost_helper.providers.nginx.run_elevated_command", mock_run_elevated)
    monkeypatch.setattr("vhost_helper.providers.apache.run_elevated_command", mock_run_elevated)
    ...
    runner.invoke(app, ["create", "managed-apache.test", str(web_root), "--provider", "apache", "--no-scaffold"], catch_exceptions=False)
    # Managed Nginx (disabled) - create then disable
    runner.invoke(app, ["create", "managed-nginx.test", str(web_root), "--provider", "nginx", "--no-scaffold"], catch_exceptions=False)
    runner.invoke(app, ["disable", "managed-nginx.test", "--provider", "nginx"], catch_exceptions=False)
...
def test_enable_disable_external_vhost(mixed_env):
    ...
    result_enable = runner.invoke(app, ["enable", "external-apache.test", "--provider", "apache"], catch_exceptions=False)
    assert result_enable.exit_code == 0
    assert "Virtual host 'external-apache.test' has been enabled." in result_enable.stdout
    ...
    assert "external-apache.test" in result_list.stdout and "Enabled" in result_list.stdout

    # 3. Disable external Apache vhost
    result_disable = runner.invoke(app, ["disable", "external-apache.test", "--provider", "apache"], catch_exceptions=False)
    assert result_disable.exit_code == 0
    assert "Virtual host 'external-apache.test' has been disabled." in result_disable.stdout
...
def test_remove_external_vhost_is_allowed(mixed_env):
    ...
    """
    result = runner.invoke(app, ["remove", "external-nginx.test", "--force"], catch_exceptions=False)
    assert result.exit_code == 0
...
def test_create_external_vhost_is_blocked(mixed_env):
    ...
    result = runner.invoke(
        app, 
        ["create", "external-apache.test", "/var/www/external-apache", "--provider", "apache"],
