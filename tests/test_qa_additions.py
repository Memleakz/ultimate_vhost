...
def test_cli_list_no_sites(mocker, tmp_path):
    # Mock all four vhost dirs to empty temp dirs so no sites are found
    empty = tmp_path / "empty"
    empty.mkdir()
    mocker.patch("vhost_helper.main.NGINX_SITES_AVAILABLE", empty)
    mocker.patch("vhost_helper.main.NGINX_SITES_ENABLED", empty)
    mocker.patch("vhost_helper.main.APACHE_SITES_AVAILABLE", empty)
    mocker.patch("vhost_helper.main.APACHE_SITES_ENABLED", empty)
...
def test_cli_remove_vhost_success(mocker, tmp_path):
    ...
    enabled_dir.mkdir()
    (available_dir / "test.test.conf").write_text("# Generated by VHost Helper\n")
