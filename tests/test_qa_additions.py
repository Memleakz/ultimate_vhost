...
def test_os_detector_failure(mocker):
    ...
    mocker.patch("subprocess.run", side_effect=subprocess.CalledProcessError(1, "detect_os.sh", stderr="OS not supported"))
...
def test_cli_info_system(mocker):
    # Mock get_os_info
    mocker.patch("vhost_helper.main.get_os_info", return_value=OSInfo(id="ubuntu", version="22.04", family="debian"))
...
def test_hostfile_shell_injection_prevention(temp_hosts_file, mocker):
    ...
    tee_call = next(
        c for c in mock_run.call_args_list if "tee" in c[0][0]
    )
    args = tee_call[0][0]
    assert "tee" in args
    assert "-a" in args
...
def test_nginx_rollback_on_reload_failure(mocker):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with pytest.raises(RuntimeError, match="Nginx reload failed, rollback performed"):
            provider.create_vhost(config)
...
def test_detect_os_script_debian_legacy(mocker):
    ...
    try:
        ...
        modified_script = script_content.replace("/etc/os-release", "/tmp/nonexistent-os-release")
        modified_script = modified_script.replace("/etc/redhat-release", "/tmp/nonexistent-redhat-release")
...
def test_detect_os_script_no_files_fail(mocker):
    try:
        ...
        modified_script = script_content.replace("/etc/os-release", "/tmp/nonexistent-os-release")
        modified_script = modified_script.replace("/etc/redhat-release", "/tmp/nonexistent-redhat-release")
        modified_script = modified_script.replace("/etc/debian_version", "/tmp/nonexistent-debian-version")
...
def test_nginx_provider_invalid_template_dir(mocker):
    ...
    mocker.patch("vhost_helper.providers.nginx.APP_TEMPLATES_DIR", Path("/nonexistent/app/templates"))
    mocker.patch("vhost_helper.providers.nginx.USER_TEMPLATES_DIR", Path("/nonexistent/user/templates"))
