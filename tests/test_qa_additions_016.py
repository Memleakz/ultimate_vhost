...
DEFAULT_TEMPLATE_CONTENT = "server { listen 80; server_name {{ domain }}; root {{ document_root }}; }"
...
def mock_provider(user_template_dir, app_template_dir, mocker):
    ...
    mocker.patch("vhost_helper.providers.nginx.NGINX_SITES_AVAILABLE", Path("/fake/sites-available"))
    mocker.patch("vhost_helper.providers.nginx.NGINX_SITES_ENABLED", Path("/fake/sites-enabled"))
...
def test_uses_default_app_template(mock_provider, vhost_config_obj):
    ...
    rendered = template.render(domain=vhost_config_obj.domain, document_root=vhost_config_obj.document_root)
    expected = DEFAULT_TEMPLATE_CONTENT.replace("{{ domain }}", vhost_config_obj.domain).replace("{{ document_root }}", str(vhost_config_obj.document_root))
    assert rendered == expected

def test_uses_custom_user_template_override(mock_provider, vhost_config_obj, user_template_dir):
    ...
    template = mock_provider._get_template("default")
    rendered = template.render(domain=vhost_config_obj.domain, document_root=vhost_config_obj.document_root)
    expected = CUSTOM_DEFAULT_TEMPLATE_CONTENT.replace("{{ domain }}", vhost_config_obj.domain).replace("{{ document_root }}", str(vhost_config_obj.document_root))
    assert rendered == expected

def test_uses_named_template_from_app_dir(mock_provider, vhost_config_obj, app_template_dir):
    ...
    rendered = template.render(domain=vhost_config_obj.domain, document_root=vhost_config_obj.document_root)
    expected = WORDPRESS_TEMPLATE_CONTENT.replace("{{ domain }}", vhost_config_obj.domain).replace("{{ document_root }}", str(vhost_config_obj.document_root))
    assert rendered == expected

def test_uses_named_template_from_user_dir(mock_provider, vhost_config_obj, user_template_dir):
...
def test_user_named_template_overrides_app_template(mock_provider, vhost_config_obj, user_template_dir, app_template_dir):
...
def test_initialize_user_config_creates_directory(tmp_path):
    ...
    with patch("pathlib.Path.home", return_value=tmp_path / "user_home"):
        ...
        with patch("vhost_helper.config.USER_TEMPLATES_DIR", user_config_dir / "templates"):
