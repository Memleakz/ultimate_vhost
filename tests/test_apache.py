...
def test_enable_vhost_rhel(rhel_patched_apache_provider, vhost_config):
    ...
    (rhel_patched_apache_provider.conf_disabled / (vhost_config.domain + ".conf")).touch()
...
def test_validate_config_debian(patched_apache_provider):
    ...
    with patch.object(apache.ApacheProvider, 'validate_config', autospec=True) as mock_validate:
