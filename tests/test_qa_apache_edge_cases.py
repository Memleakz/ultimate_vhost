...
def mock_apache_dirs(tmp_path):
    ...
    with patch("vhost_helper.providers.apache.APACHE_SITES_AVAILABLE", available), \
         patch("vhost_helper.providers.apache.APACHE_SITES_ENABLED", enabled), \
...
def mock_apache_dirs_rhel(tmp_path):
    ...
    with patch("vhost_helper.providers.apache.APACHE_SITES_AVAILABLE", conf_d), \
         patch("vhost_helper.providers.apache.APACHE_SITES_ENABLED", conf_d), \
         patch("vhost_helper.providers.apache.APACHE_SITES_DISABLED", disabled), \
         patch("vhost_helper.providers.apache.detected_os_family", "rhel_family"):
...
def test_apache_template_log_dir_rhel(mock_apache_dirs_rhel, tmp_path):
    ...
    with patch("vhost_helper.providers.apache.run_elevated_command") as mock_run, \
         patch("vhost_helper.providers.apache.get_sudo_prefix", return_value=[]), \
    ...
    if "${APACHE_LOG_DIR}" in content:
        pytest.fail("Template uses ${APACHE_LOG_DIR} which is Debian-specific and likely fails on RHEL/Fedora.")
