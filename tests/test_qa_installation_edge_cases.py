...
def test_install_script_missing_binary(project_paths, tmp_path):
    ...
    assert "Error: Could not find vhost binary" in result.stdout or "Error: Could not find vhost binary" in result.stderr
...
def test_uninstall_deep_clean_safety(project_paths, tmp_path):
    ...
    test_script.write_text(content.replace('if [ "$EUID" -ne 0 ]; then', 'if false; then'))
...
def test_install_script_idempotency(project_paths, tmp_path):
    ...
    content = content.replace('/opt/vhost-helper', str(tmp_path / "opt" / "vhost-helper"))
    ...
    content = content.replace('/usr/local/bin/vhost --show-completion', 'echo mock-completion')
