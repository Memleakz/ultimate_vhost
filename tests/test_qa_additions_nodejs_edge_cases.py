...
def test_node_port_boundary_min(tmp_path):
    ...
    doc_root.mkdir()
    config = VHostConfig(
        domain="node.local", document_root=str(doc_root),
        server_type=ServerType.NGINX, runtime=RuntimeMode.NODEJS, node_port=1,
    )
...
def test_node_port_boundary_max(tmp_path):
    ...
    config = VHostConfig(
        domain="node.local", document_root=str(doc_root),
        server_type=ServerType.NGINX, runtime=RuntimeMode.NODEJS, node_port=65535,
    )
...
def test_node_port_zero_rejected(tmp_path):
    ...
    with pytest.raises(ValidationError):
        VHostConfig(
            domain="node.local", document_root=str(doc_root),
            server_type=ServerType.NGINX, runtime=RuntimeMode.NODEJS, node_port=0,
        )
...
def test_node_port_above_max_rejected(tmp_path):
    ...
    with pytest.raises(ValidationError):
        VHostConfig(
            domain="node.local", document_root=str(doc_root),
            server_type=ServerType.NGINX, runtime=RuntimeMode.NODEJS, node_port=65536,
        )
...
def test_node_socket_empty_string(tmp_path):
    ...
    config = VHostConfig(
        domain="node.local", document_root=str(doc_root),
        server_type=ServerType.NGINX, runtime=RuntimeMode.NODEJS,
        node_socket="",
    )
    ...
    rendered = template.render(
        domain=config.domain, document_root=str(config.document_root),
        port=config.port, runtime=config.runtime.value,
        node_port=config.node_port, node_socket=config.node_socket,
        php_socket=config.php_socket, python_port=config.python_port,
...
def test_node_socket_with_spaces_in_path(tmp_path):
    ...
    rendered = template.render(
        domain="node.local", document_root=str(doc_root),
        port=80, runtime="nodejs", node_port=3000, node_socket=socket_path,
        php_socket="/run/php/php-fpm.sock", python_port=8000,
        os_family="debian_family",
    )
...
def test_nginx_nodejs_proxy_uses_fixed_log_path(tmp_path):
    ...
    template = provider._get_template("nodejs-proxy")
    rendered = template.render(
        domain="node.local", document_root=str(tmp_path),
        port=80, runtime="nodejs", node_port=3000, node_socket=None,
        php_socket="/run/php/php-fpm.sock", python_port=8000,
...
def test_nginx_nodejs_canonical_redirect_custom_port(tmp_path):
    ...
    rendered = template.render(
        domain="node.local", document_root=str(tmp_path),
        port=8888, runtime="nodejs", node_port=3000, node_socket=None,
        php_socket="/run/php/php-fpm.sock", python_port=8000,
        os_family="debian_family",
    )
...
def test_apache_nodejs_redirect_www_to_bare(tmp_path):
    ...
    template = provider._get_template("nodejs-proxy")
    rendered = template.render(
        domain="www.node.local", document_root=str(tmp_path),
        port=80, runtime="nodejs", node_port=3000, node_socket=None,
        php_socket="/run/php/php-fpm.sock", python_port=8000,
        os_family="debian_family",
    )
...
def test_apache_nodejs_uds_format_includes_pipe(tmp_path):
    ...
    rendered = template.render(
        domain="node.local", document_root=str(tmp_path),
        port=80, runtime="nodejs", node_port=3000, node_socket=sock,
        php_socket="/run/php/php-fpm.sock", python_port=8000,
        os_family="debian_family",
    )
...
def test_cli_node_port_without_nodejs_runtime_ignored(mock_nginx_setup, tmp_path):
    ...
    result = runner.invoke(app, [
        "create", "site.local", str(doc_root),
        "--provider", "nginx",
        "--node-port", "9000",
    ])
...
def test_cli_nodejs_runtime_option_same_as_flag(mock_nginx_setup, tmp_path):
    ...
    doc_root = tmp_path / "www"
    doc_root.mkdir()
    result_flag = runner.invoke(app, [
        "create", "node-a.local", str(doc_root),
        "--provider", "nginx", "--nodejs",
    ])
    ...
    result_opt = runner.invoke(app, [
        "create", "node-b.local", str(doc_root),
        "--provider", "nginx", "--runtime", "nodejs",
    ])
...
def test_cli_all_three_runtime_flags_mutually_exclusive(mock_nginx_setup, tmp_path):
    ...
    doc_root.mkdir()
    result = runner.invoke(app, [
        "create", "node.local", str(doc_root),
        "--provider", "nginx",
        "--php", "--python", "--nodejs",
    ])
