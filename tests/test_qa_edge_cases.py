...
def mock_privileged_ops(mocker, tmp_path):
    ...
    mocker.patch("vhost_helper.utils.run_elevated_command", return_value=subprocess.CompletedProcess([], 0))
    ...
    mocker.patch("vhost_helper.providers.nginx.run_elevated_command", return_value=subprocess.CompletedProcess([], 0))
...
def test_vhost_config_document_root_not_exists():
    with pytest.raises(ValidationError) as excinfo:
        VHostConfig(
            domain="example.test",
            document_root=Path('/non/existent/path/at/all')
        )
...
def test_vhost_config_invalid_document_root_injection(mocker):
    ...
    with pytest.raises(ValidationError) as excinfo:
        VHostConfig(
            domain="example.test",
            document_root=Path('/tmp/dir"withquote')
        )
...
def test_cli_info_generic_error(mocker, tmp_path):
    ...
    result = runner.invoke(app, ["info", "example.test"])
    assert "Error reading configuration: Generic Error" in result.output

def test_cli_info_os_detection_error(mocker):
    mocker.patch("vhost_helper.main.get_os_info", side_effect=Exception("OS detection failure"))
...
def test_cli_create_nginx_notification_when_stopped(mocker, tmp_path):
    ...
    mocker.patch("vhost_helper.providers.nginx.NginxProvider.create_vhost", return_value=None)
...
def test_cli_remove_nginx_notification_when_stopped(mocker, tmp_path):
    nginx_avail = tmp_path / "nginx-available"
    nginx_avail.mkdir(exist_ok=True)
    (nginx_avail / "example.test.conf").touch()

    mocker.patch("vhost_helper.main.is_nginx_running", return_value=False)
    mocker.patch("typer.confirm", return_value=True)
    mocker.patch("vhost_helper.providers.nginx.NginxProvider.remove_vhost", return_value=None)
...
def test_cli_remove_generic_error(mocker, tmp_path):
    ...
    mocker.patch("typer.confirm", return_value=True)
    mocker.patch("vhost_helper.main.remove_entry", side_effect=Exception("Remove Error"))
    result = runner.invoke(app, ["remove", "example.test"])
    assert result.exit_code != 0
...
def test_nginx_reload_failure_both_methods(mocker):
    ...
    mocker.patch("vhost_helper.providers.nginx.run_elevated_command", side_effect=RuntimeError("Both failed"))
    mocker.patch("pathlib.Path.exists", return_value=True)
    mocker.patch("pathlib.Path.is_dir", return_value=True)
...
def test_nginx_remove_vhost_failure(mocker):
    from vhost_helper.providers.nginx import NginxProvider
    mocker.patch("vhost_helper.providers.nginx.run_elevated_command", side_effect=RuntimeError("Remove failed"))
    mocker.patch("pathlib.Path.exists", return_value=True)
    provider = NginxProvider()
...
def test_hostfile_rollback_failure_logs_error(mocker, tmp_path):
    mocker.patch("vhost_helper.main.NginxProvider.create_vhost", side_effect=Exception("Nginx Error"))
    mocker.patch("vhost_helper.main.remove_entry", side_effect=Exception("Rollback Error"))
    result = runner.invoke(app, ["create", "example.test", str(tmp_path)])
    assert result.exit_code != 0
    assert "Error during hostfile rollback: Rollback Error" in result.output

def test_config_env_fallback(mocker):
    from vhost_helper import config
    mocker.patch.dict(os.environ, {"VHOST_TEST_MODE": "1", "VHOST_HOSTS_FILE": "/tmp/custom_hosts"})
