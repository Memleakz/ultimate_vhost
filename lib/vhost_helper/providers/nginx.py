...
def _extract_nginx_vhost_details(
    config_content: str, config_path: Path
) -> Tuple[Optional[str], Optional[Path]]:
    """
    Extracts the domain (server_name) and document root from Nginx config content.
    Returns (domain, document_root) or (None, None) if not found.
    Handles multiple server_name directives in a single file by returning the first.
    """
    domain = None
    document_root = None

    # Try to find server_name. This can be one or more space-separated domains.
    # We'll take the first one as the primary domain for listing.
    ...
    if root_match:
        ...
        root_path_str = root_match.group(1).strip().strip('"\'')
        document_root = Path(root_path_str)

    # Fallback if domain is not found, use filename
    if domain is None and config_path.stem != "default":  # Avoid using 'default' as a domain
...
class NginxProvider:
    ...
    def list_all_vhosts(self) -> List[VHostInfo]:
        """
        Discovers and returns a list of all Nginx virtual hosts on the system.
        ...
        scan_dirs = {NGINX_SITES_AVAILABLE, NGINX_SITES_ENABLED}
        if self.os_family == "rhel_family" and NGINX_SITES_DISABLED and NGINX_SITES_DISABLED.exists():
            scan_dirs.add(NGINX_SITES_DISABLED)
        ...
        for directory in scan_dirs:
            ...
            for config_path in directory.iterdir():
                ...
                try:
                    ...
                    domain, doc_root = _extract_nginx_vhost_details(content, real_path)
    ...
    def enable_vhost(self, config_file_path, service_running: bool = True):
        ...
        if isinstance(config_file_path, str):
            ...
            config_file_path = NGINX_SITES_AVAILABLE / (domain + ".conf")
        else:
        ...
        if self.os_family == "rhel_family":
            ...
            # For RHEL, config_file_path is expected to be in NGINX_SITES_DISABLED
            # or directly in NGINX_SITES_AVAILABLE if it's a new external file.
            # We assume config_file_path points to the 'available' version or similar.
            enabled_config_target = NGINX_SITES_ENABLED / (domain + ".conf")
            ...
            if config_file_path.parent == NGINX_SITES_AVAILABLE:
                source_path = config_file_path
            elif config_file_path.parent == NGINX_SITES_DISABLED:
                source_path = config_file_path
            else:
            ...
            cmd_mv = get_sudo_prefix() + ["mv", str(source_path), str(enabled_config_target)]
            run_elevated_command(cmd_mv)

        else:  # Debian family
            # For Debian, config_file_path is expected to be in NGINX_SITES_AVAILABLE
            # or an external location. We create a symlink to it in NGINX_SITES_ENABLED.
            enabled_link = NGINX_SITES_ENABLED / (domain + ".conf")

            if not config_file_path.exists():
                raise FileNotFoundError(f"Configuration file not found at {config_file_path}.")
            link_exists = enabled_link.exists()
            if link_exists and enabled_link.is_symlink() and enabled_link.resolve() == config_file_path:
                return  # Already enabled and correctly linked
            elif link_exists:
                # If a file or broken symlink exists, remove it before creating a new one
                run_elevated_command(get_sudo_prefix() + ["rm", "-f", str(enabled_link)])
    ...
    def disable_vhost(self, config_file_path, service_running: bool = True):
        ...
        if self.os_family == "rhel_family":
            ...
            # For RHEL, config_file_path is expected to be in NGINX_SITES_ENABLED
            # or an external location. We move it to NGINX_SITES_DISABLED.
            ...
            # For Debian, we remove the symlink from NGINX_SITES_ENABLED.
            enabled_link = NGINX_SITES_ENABLED / (domain + ".conf")
