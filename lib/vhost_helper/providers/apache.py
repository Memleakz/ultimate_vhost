...
def _extract_apache_vhost_details(
    ...
    server_name_match = re.search(r"ServerName\s+([^\s:]+)", config_content, re.IGNORECASE)
    if server_name_match:
        domain = server_name_match.group(1).strip()

    # Find DocumentRoot directive.
    document_root_match = re.search(r"DocumentRoot\s+([^\s]+)", config_content, re.IGNORECASE)
    ...
    if domain is None and config_path.stem != "000-default": # Avoid using '000-default' as a domain
...
class ApacheProvider:
    ...
    def list_all_vhosts(self) -> List[VHostInfo]:
        ...
        if self.os_family == "rhel_family" and APACHE_SITES_DISABLED and APACHE_SITES_DISABLED.exists():
