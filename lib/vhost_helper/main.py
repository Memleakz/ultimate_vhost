...
from .config import (
    NGINX_SITES_AVAILABLE, NGINX_SITES_ENABLED, NGINX_SITES_DISABLED,
    APACHE_SITES_AVAILABLE, APACHE_SITES_ENABLED, APACHE_SITES_DISABLED,
    APP_TEMPLATES_DIR,
    initialize_user_config
)
from .models import VHostConfig, ServerType, RuntimeMode, PHP_SOCKET_PATHS, DEFAULT_PHP_SOCKET
...
from .template_inspector import list_templates, resolve_template_path, extract_variables, extract_metadata
...
def validate_domain(domain: str) -> str:
    ...
    if any(len(label) > 63 for label in domain.split(".")):
        raise ValueError(f"Domain label exceeds 63-character limit (RFC 1035): '{domain}'")
    if not re.match(DOMAIN_REGEX, domain):
        raise ValueError(f"Invalid domain format: '{domain}'. Use only alphanumeric characters, dots, and hyphens. (Must be at least 3 characters)")
    return domain
...
def create(
    ...
    document_root: Path = typer.Argument(..., help="The absolute path to the project root"),
    provider: Optional[ServerType] = typer.Option(None, "--provider", "-p", help="Web server provider (nginx or apache)"),
    port: int = typer.Option(80, help="Port number"),
    php: bool = typer.Option(False, "--php", help="Enable PHP (php-fpm) support"),
    python: bool = typer.Option(False, "--python", help="Enable Python (gunicorn) support"),
    python_port: int = typer.Option(8000, "--python-port", help="Gunicorn upstream port (used with --python)"),
    nodejs: bool = typer.Option(False, "--nodejs", help="Enable Node.js reverse proxy support"),
    node_port: int = typer.Option(3000, "--node-port", help="Node.js upstream port (used with --nodejs or --runtime nodejs)"),
    node_socket: Optional[str] = typer.Option(None, "--node-socket", help="Unix Domain Socket path for Node.js (overrides --node-port)"),
    runtime_opt: Optional[RuntimeMode] = typer.Option(None, "--runtime", help="Runtime mode (static, php, python, nodejs)"),
    template: str = typer.Option("default", "--template", "-t", help="Name of the template to use (e.g., 'wordpress')"),
):
    ...
    if active_flags > 1:
        console.print("[red]✖[/red] Error: --php, --python, --nodejs, and --runtime are mutually exclusive.")
        raise typer.Exit(code=1)

    console.print(f"[bold blue]\\[vhost][/bold blue] Provisioning '[cyan]{domain}[/cyan]'...")
    ...
    if not document_root.exists():
        console.print(f"  [red]✖[/red] Error: Document root [cyan]{document_root}[/cyan] does not exist.")
        raise typer.Exit(code=1)
    ...
    try:
        ...
        console.print(f"  [green]✔[/green] Hostfile entries added (127.0.0.1 {domain}, {get_redirect_domain(domain)})")

        # 3. Create vhost
        provider_instance = _get_provider(server_type)
        with _tracked_status(f"[bold green]Configuring {server_name}...", spinner="dots"):
        ...
        if server_type == ServerType.NGINX or (server_type == ServerType.APACHE and APACHE_SITES_ENABLED != APACHE_SITES_AVAILABLE):
             console.print(f"  [green]✔[/green] Symbolic link created")
        ...
        if service_running:
            ...
            console.print(f"  [dim]⊘ Skipped: {server_name} configuration validation ({server_name} service is not running)[/dim]")
            console.print(f"  [dim]⊘ Skipped: {server_name} reload ({server_name} service is not running)[/dim]")
        ...
        if hostfile_updated:
            try:
                ...
                console.print(f"  [yellow]↪[/yellow] Rollback: Hostfile entry removed due to failure.")
            except Exception as rollback_err:
                console.print(f"  [red]✖[/red] Error during hostfile rollback: {rollback_err}")
...
def remove(
    ...
    console.print(f"[bold blue]\\[vhost][/bold blue] Removing '[cyan]{domain}[/cyan]'...")
    ...
    if provider is None:
        ...
        if server_type is None:
            console.print(f"  [red]✖[/red] Error: No configuration found for [cyan]{domain}[/cyan]")
            raise typer.Exit(code=1)
    else:
        server_type = provider
    ...
    try:
        ...
        with _tracked_status("[bold green]Updating hostfile...", spinner="dots"):
            remove_entry(domain)
            remove_entry(get_redirect_domain(domain))
        console.print(f"  [green]✔[/green] Hostfile entries removed")

        # 2. Remove vhost
        provider_instance = _get_provider(server_type)
        with _tracked_status(f"[bold green]Removing {server_name} configuration...", spinner="dots"):
...
def enable(
    ...
    console.print(f"[bold blue]\\[vhost][/bold blue] Enabling '[cyan]{domain}[/cyan]'...")
    ...
    if enabled_path.exists():
        console.print(f"  [yellow]ℹ[/yellow] Virtual host '{domain}' is already enabled for {server_name}.")
        raise typer.Exit()
    ...
    try:
        ...
        provider_instance = _get_provider(server_type)
        with _tracked_status(f"[bold green]Enabling {server_name} configuration...", spinner="dots"):
...
def disable(
    ...
    console.print(f"[bold blue]\\[vhost][/bold blue] Disabling '[cyan]{domain}[/cyan]'...")
    ...
    if not enabled_path.exists() and not enabled_path.is_symlink():
        console.print(f"  [yellow]ℹ[/yellow] Virtual host '{domain}' is already disabled for {server_name}.")
        raise typer.Exit()
    ...
    try:
        ...
        provider_instance = _get_provider(server_type)
        with _tracked_status(f"[bold green]Disabling {server_name} configuration...", spinner="dots"):
        ...
        if directory.exists():
            for config_file in directory.iterdir():
                if config_file.is_file() and config_file.suffix == ".conf":
                    ...
                    status = "[green]Enabled[/green]" if enabled_link.exists() else "[yellow]Disabled[/yellow]"
...
@app.command()
def info(
    domain: Optional[str] = typer.Argument(None, help="The domain name to show info for"),
    provider: Optional[ServerType] = typer.Option(None, "--provider", "-p", help="Web server provider (nginx or apache)"),
):
    ...
    if domain:
        ...
        if config_path.exists():
            try:
                ...
                server_name = server_name_match.group(1).replace('"', '') if server_name_match else domain
                root_path = root_match.group(1) if root_match else "Unknown"
                port_info = listen_match.group(1).replace('"', '') if listen_match else "80"
                ...
                console.print(f"[red]✖[/red] Error: Permission denied reading configuration. Try running with [bold]sudo[/bold].")
            except Exception as e:
...
def template_vars():
    ...
    rows = [
        ("domain",        "str",     "required",        "nginx, apache", "Primary domain name (e.g. [cyan]example.com[/cyan])"),
        ("document_root", "str",     "required",        "nginx, apache", "Absolute path to the web root directory"),
        ("port",          "int",     "[green]80[/green]",       "nginx, apache", "TCP port the server listens on"),
        ("runtime",       "str",     "[green]static[/green]",   "nginx, apache", "Runtime mode: [bold]static[/bold] | [bold]php[/bold] | [bold]python[/bold] | [bold]nodejs[/bold]"),
        ("python_port",   "int",     "[green]8000[/green]",     "nginx, apache", "Local port gunicorn/uvicorn listens on (used when runtime=python)"),
        ("node_port",     "int",     "[green]3000[/green]",     "nginx, apache", "Local port the Node.js process listens on (used when runtime=nodejs)"),
        ("node_socket",   "str",     "[green]None[/green]",     "nginx, apache", "Unix Domain Socket path for Node.js — overrides node_port when set"),
        ("php_socket",    "str",     "[green]/run/php/php-fpm.sock[/green]", "nginx, apache", "PHP-FPM unix socket path (used when runtime=php)"),
        ("os_family",     "str",     "auto-detected",   "nginx, apache", "OS family: [bold]debian_family[/bold] | [bold]rhel_family[/bold] (controls provider-specific config paths)"),
    ]
    ...
    runtime_table = Table(show_header=True, header_style="bold", border_style="dim", box=None, padding=(0, 2))
    runtime_table.add_column("Value", style="bold cyan", no_wrap=True)
    runtime_table.add_column("Behaviour")
    runtime_table.add_row("static",  "Serves static files; try_files with index.html fallback")
    runtime_table.add_row("php",     "FastCGI pass to [green]{{ php_socket }}[/green]")
    runtime_table.add_row("python",  "Reverse proxy to [green]http://127.0.0.1:{{ python_port }}[/green]")
    runtime_table.add_row("nodejs",  "Reverse proxy to [green]http://127.0.0.1:{{ node_port }}[/green] (or [green]{{ node_socket }}[/green] when set)")
...
def templates_list(
    provider: Optional[str] = typer.Option(None, "--provider", help="Filter by provider (nginx or apache)"),
):
    ...
    if not result:
        if provider:
            console.print(f"[red]✖[/red] No templates found for provider '[cyan]{provider}[/cyan]'.")
        else:
...
def templates_inspect(
    name: str = typer.Argument(..., help="Template name in <provider>-<mode> format (e.g. nginx-php)"),
):
    ...
    if template_path is None:
        console.print(f"[red]✖[/red] Template '[cyan]{name}[/cyan]' not found in {templates_dir}.")
        raise typer.Exit(code=1)
